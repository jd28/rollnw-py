...
void init_component_appearance(nb::module_& m)
{
    nb::enum_<nw::CreatureColors::type>(m, "CreatureColors")
    ...
    nb::class_<nw::BodyParts>(m, "BodyParts")
        .def(nb::init<>())
    ...
    nb::class_<nw::CreatureAppearance>(m, "Appearance")
        .def(nb::init<>())
...
void init_component_combatinfo(nb::module_& m)
{
    nb::class_<nw::CombatInfo>(m, "CombatInfo")
...
void init_component_common(nb::module_& m)
{
    nb::class_<nw::Common>(m, "Common")
...
void init_component_creature_stats(nb::module_& m)
{
    nb::class_<nw::CreatureStats>(m, "CreatureStats")
...
void init_component_equips(nb::module_& m)
{
    nb::enum_<nw::EquipSlot>(m, "EquipSlot")
    ...
    nb::enum_<nw::EquipIndex>(m, "EquipIndex")
    ...
    nb::class_<nw::Equips>(m, "Equips")
        .def_ro("equips", &nw::Equips::equips)
        .def("instantiate", &nw::Equips::instantiate);
}

void init_component_inventory(nb::module_& m)
{
    nb::class_<nw::InventoryItem>(m, "InventoryItem")
    ...
    nb::class_<nw::Inventory>(m, "Inventory")
        .def("__len__", &nw::Inventory::size)
        .def("instantiate", &nw::Inventory::instantiate)

        .def("add_item", &nw::Inventory::add_item, nb::keep_alive<1, 2>())
        ...
        .def("items", [](const nw::Inventory& self) {
            ...
            for (auto& item : self.items) {
                if (item.item.is<nw::Item*>()) {
                    auto pyobj = nb::cast(item.item.as<nw::Item*>(), nb::rv_policy::reference);
                    pylist.append(pyobj);
                }
            }
            return pylist;
...
void init_component_levelhistory(nb::module_& m)
{
    nb::class_<nw::LevelUp>(m, "LevelUp")
...
void init_component_location(nb::module_& m)
{
    nb::class_<nw::Location>(m, "Location")
        .def(nb::init<>())
...
void init_component_lock(nb::module_& m)
{
    nb::class_<nw::Lock>(m, "Lock")
...
void init_component_trap(nb::module_& m)
{
    nb::class_<nw::Trap>(m, "Trap")
        .def(nb::init<>())
...
void init_object_components(nb::module_& m)
{
